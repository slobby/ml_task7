...
def test_gini_gain(func):
    def test(x, xs, value):
        assert np.isclose(
            func(x, xs), value), f'Should be {value} for {x} and {xs}'
...
def test_entropy(func):
    assert func(
        np.array([0])) == 0.0, 'Should be 0.0 if all elements are equal'
    assert func(
        np.array([1])) == 0.0, 'Should be 0.0 if all elements are equal'
...
def test_split_dataset(func):
    ...
    assert np.allclose(
        l_X, [[-0.27279702,  0.06677798], [-1.76050488,  1.08797011]])
    assert np.array_equal(l_y, [1, 1])
    l_X, r_X, l_y, r_y = func(df, trgt, 1, 0.1)
    assert l_X is not None and l_y is not None, "Should not return None values"
    assert np.allclose(
        l_X, [[1.33254869, -1.41820464], [-0.27279702,  0.06677798]])
